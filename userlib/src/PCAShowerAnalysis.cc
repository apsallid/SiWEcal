...
void PCAShowerAnalysis::showerParameters(const SiWEcalSSCluster & clus)
{

  if (!alreadyfilled_) {
    
    double variables[3] = {0.,0.,0.};
    const std::map<SiWEcalSSRecoHit*,double> & lmap = clus.recHitFractions();
    std::map<SiWEcalSSRecoHit*,double>::const_iterator iter = lmap.begin();
    
    if (debug_) std::cout << " -- Number of rechits in cluster: " << clus.nRecHits() << " " << lmap.size() << std::endl;
    unsigned counter = 0;
    for (;iter!=lmap.end();++iter){
      SiWEcalSSRecoHit* myhit = iter->first;
      if (!myhit) {
