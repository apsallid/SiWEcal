...
SiWEcalSSRecoHit::SiWEcalSSRecoHit(const SiWEcalSSSimHit & aSimHit, const unsigned granularity){
...
double SiWEcalSSRecoHit::theta() const {
  return 2*atan(exp(-1.*eta()));
}

double SiWEcalSSRecoHit::eta() const {
  return position().eta();
}

double SiWEcalSSRecoHit::phi() const {
  return position().phi();
}

// void SiWEcalSSRecoHit::encodeCellId(const bool x_side,const bool y_side,const unsigned x_cell,const unsigned y_cell, const unsigned granularity){
//   cellid_ = 
...
void SiWEcalSSRecoHit::Add(const SiWEcalSSSimHit & aSimHit){
  time_ = time_*energy_;
  energy_ += aSimHit.energy();
...
void SiWEcalSSRecoHit::Print(std::ostream & aOs) const{
  aOs << "====================================" << std::endl
