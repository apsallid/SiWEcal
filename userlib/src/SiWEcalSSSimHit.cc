...
SiWEcalSSSimHit::SiWEcalSSSimHit(const G4SiHit & aSiHit, const unsigned & asilayer, const float cellSize){
...
void SiWEcalSSSimHit::encodeCellId(const bool x_side,const bool y_side,const unsigned x_cell,const unsigned y_cell){
  cellid_ = 
    x_side | (x_cell<<1) |
    (y_side<<16) | (y_cell<<17);
...
void SiWEcalSSSimHit::Add(const G4SiHit & aSiHit){
...
/*double SiWEcalSSSimHit::eta() const {
  double x = get_x();
  double y = get_y();
...
double SiWEcalSSSimHit::theta() const {
...
double SiWEcalSSSimHit::phi() const {
  return position().phi();
}

void SiWEcalSSSimHit::Print(std::ostream & aOs) const{
  aOs << "====================================" << std::endl
