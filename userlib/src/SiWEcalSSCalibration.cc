...
SiWEcalSSCalibration::SiWEcalSSCalibration(std::string filePath,
				   const bool bypassR,
				   const unsigned nSi){
...
SiWEcalSSCalibration::~SiWEcalSSCalibration(){
}

double SiWEcalSSCalibration::addTimeOfFlight(const double & aTime,
					 const double & posx,
					 const double & posy,
					 const double & posz){
...
double SiWEcalSSCalibration::correctTime(const double & aTime,
				     const double & posx,
				     const double & posy,
				     const double & posz){
...
double SiWEcalSSCalibration::MeVToMip(const unsigned layer, const bool absWeight) const{
...
/*double SiWEcalSSCalibration::MeVToMip(const unsigned layer, const double aEta, const bool absWeight) const{
  double res = 1;
  if (layer < theDetector().nLayers())
...
double SiWEcalSSCalibration::MeVToMip(const unsigned layer, const double aRadius, const bool absWeight) const{
  double res = 1;
  if (layer < theDetector().nLayers())
    res = theDetector().subDetectorByLayer(layer).mipWeight
      *(absWeight?theDetector().subDetectorByLayer(layer).absWeight : 1.0);
