#ifndef SiWEcalSSGeometryConversion_h
#define SiWEcalSSGeometryConversion_h
...
class SiWEcalSSGeometryConversion{
  
public:
  SiWEcalSSGeometryConversion(){};
  SiWEcalSSGeometryConversion(std::string filePath,const unsigned & model, const double & cellsize, const bool bypassR=false, const unsigned nSiLayers=3);

  ~SiWEcalSSGeometryConversion();

  void setGranularity(const std::vector<unsigned> & granul);

  unsigned getGranularity(const unsigned aLayer, const SiWEcalSSSubDetector & adet);
